...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((l (get-line-segment)) (lstop (get-end-point l)) (lstart (get-start-point l)) (x (car lstart)) (y (cdr lstart)) (z (car lstop)) (t (cdr lstop)))
    (sqrt (+ (sqr (- x y)) (sqr (- z t))))))
...
(define (compute-f-with-step f a b step)
  (let name ((a a) (result '()))             ; parametrul b este inițializat cu valoarea b, result cu '()  
   (if (> a b)                              ; aici e vizibil b-ul parametru pentru iter, nu b-ul funcției interval  
       (reverse result)  
       (name (+ a step) (cons (f a) result)))))
...
(define (num-concat x y)
  (let* ((a (number->string x)) (b (number->string y)) (c (string-append a b)) (+ (string-append)))
