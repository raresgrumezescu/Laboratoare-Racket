...
(define (num->base n b)
  (if (= n 0)
      null
      (append (num->base (quotient n b) b) (cons (modulo n b) null))))
...
(define (rev L)
  (if (null? L) null (append (rev (cdr L)) (cons (car L) null))))
...
(define (deep-rev L)
  (if (null? L)
      null
      (cond
        ((list? (car L)) (append (deep-rev (cdr L)) (cons (deep-rev (car L) ) null)))
        (else (append (deep-rev (cdr L)) (cons (car L) null))))))
...
(define (palindromes-to-n n Bases)
  (if (= n -1)
      null
      (if (and (all-palindromes? n Bases) #t)
          (append (palindromes-to-n (- n 1) Bases) (cons n null))
          (append (palindromes-to-n (- n 1) Bases) null) )))
...
;; functie care primeste lista (2 3 4 5 6 7 8 9 10)
;; si formeaza o lista cu numerele in baza carora n este palindrom 
(define (in-what-bases n Bases)
  (if (equal? Bases null)
      null
      (if (palindrome? (num->base n (car Bases)))
          (append (in-what-bases n (cdr Bases)) (cons (car Bases) null))
          (append (in-what-bases n (cdr Bases)) null))))

(define (first-b-pal start b)
  (if (>= (length (in-what-bases (add1 start) (range 2 11))) b)
      (add1 start)
...
;; functie care imi transforma un numar in lista
(define (num-to-list n)
  (if (= n 0)
      null
      (append (num-to-list (quotient n 10)) (cons (modulo n 10) null))))

(define (longest-palindrome n)
   (if (palindrome? (num-to-list n))
       n
       (if (<= (longest-palindrome (list->num (take (num-to-list n) (- (length (num-to-list n) ) 1))))
          (longest-palindrome (list->num (cdr (num-to-list n) ))))
          (longest-palindrome (list->num (cdr (num-to-list n) )))
          (longest-palindrome (list->num (take (num-to-list n) (- (length (num-to-list n) ) 1)))))))
